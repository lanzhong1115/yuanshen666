...
export const api = {
  ...
  searchFunds: (q) => request(`/funds/search?query=${encodeURIComponent(q)}`), getFundNav: (code, days = 90) => request(`/funds/${code}/nav?days=${days}`),
  getFundEstimate: (code) => request(`/funds/${code}/estimate`), getFundDetail: (code, withEstimate = false, days = 365) => request(`/funds/${code}/detail?include_estimate=${withEstimate}&days=${days}`),
  getIndices: () => request('/market/indices'), getSectors: () => request('/market/sectors'), syncAll: () => request('/sync/all'),
  ...
  getDailyProfit: (days = 30) => request(`/analysis/profit/daily?days=${days}`), getMonthlyProfit: (months = 12) => request(`/analysis/profit/monthly?months=${months}`),
