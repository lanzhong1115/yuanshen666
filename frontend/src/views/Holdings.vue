      ...
      <div v-for="h in holdings" :key="h.id">
        <div class="holding-row">
          ...
          <div class="h-right">
            ...
            <div class="h-daily" :class="(h.daily_return||0)>=0?'up':'down'">{{ h.valuation_time?'实时':h.nav_date?h.nav_date.slice(5):'' }} {{ (h.daily_return||0)>=0?'+':'' }}{{ (h.daily_return||0).toFixed(2) }}%</div>
          </div>
